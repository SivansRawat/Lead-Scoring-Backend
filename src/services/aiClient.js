...
const apiKey = process.env.OPENAI_API_KEY;
if (!apiKey) console.warn("OPENAI_API_KEY not set!");

export const client = new OpenAI({ apiKey });

export async function askAIForIntent(lead, offer) {
  if (!client) {
    return { intent: "Medium", reasoning: "No API key provided." };
  }

  const prompt = [
    "You are a concise sales-intent classifier. Given a product/offer and a single prospect, classify their buying intent as High, Medium, or Low and give a 1-2 sentence reasoning.",
    'Respond in JSON only: {"intent":"High","reasoning":"..."}',
    `Offer: ${JSON.stringify(offer)}`,
    `Prospect: ${JSON.stringify(lead)}`
  ].join("\n\n");
  ...
  try {
    ...
    if (!text) return { intent: "Medium", reasoning: "AI returned no text." };

    const jsonMatch = text.match(/\{[\s\S]*\}/);
    const parsed = JSON.parse(jsonMatch ? jsonMatch[0] : text);
    parsed.intent = parsed.intent || "Medium";
    return parsed;
  } catch (err) {
    console.error("AI error:", err);
    return { intent: "Medium", reasoning: `AI error: ${err.message || err}` };
  }
}
